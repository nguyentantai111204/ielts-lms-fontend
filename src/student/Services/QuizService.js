...
const API_URL_BACKUP = "http://localhost:8080/api/quiz";
export const getActiveQuizByLesson = async (lessonId) => {
  const token = localStorage.getItem("token");
  if (!token) throw new Error("Bạn chưa đăng nhập");

  try {
    const response = await axios.get(`${API_URL_BACKUP}/active_quiz_by_lesson/${lessonId}`, {
...
export const getQuizQuestions = async (quizId) => {
  ...
  try {
    const response = await axios.get(`${API_URL_BACKUP}/get_quiz_questions/${quizId}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    return response.data;
  }catch{
...
export const submitQuiz = async (gradingDto) => {
  ...
  try {
    const response = await axios.post(`${API_URL_BACKUP}/grade_quiz`, gradingDto, {
      headers: {
...
export const getQuizGradeByStudent = async (quizId, studentId) => {
  ...
  try {
    const response = await axios.get(`${API_URL_BACKUP}/get_quiz_grade/${quizId}/student/${studentId}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    return response.data;
  } catch (error) {
...
export const getAllQuizGrades = async (quizId) => {
  ...
  try {
    const response = await axios.get(`${API_URL_BACKUP}/grades/${quizId}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    return response.data;
  } catch (error) {
