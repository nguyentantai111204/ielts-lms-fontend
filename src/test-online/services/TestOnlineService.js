...
const TEST_API_URL_BACKUP = "http://localhost:8080/api/tests";
const getAuthHeader = () => {
...
export const loadTestList = async () => {
  const res = await axios.get(`${TEST_API_URL_BACKUP}/all_tests_preview`);
  return res.data;
};

// Lấy preview 1 bài test
export const getTestPreview = async (testId) => {
  const res = await axios.get(`${TEST_API_URL_BACKUP}/${testId}/preview`);
  return res.data;
};

// Lấy chi tiết full bài test
export const getTestDetail = async (testId) => {
  const res = await axios.get(`${TEST_API_URL_BACKUP}/${testId}`, { headers: getAuthHeader() });
  return res.data;
};
